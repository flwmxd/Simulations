...
namespace bvh11
{
	...
	public:
		...
		const Eigen::MatrixXf& motion()   const { return motion_; }
		auto & motion()    { return motion_; }

		std::shared_ptr<const Joint> root_joint() const { return root_joint_; }
		std::shared_ptr<Joint> root_joint() { return root_joint_; }
		...
		Eigen::Affine3f GetTransformationRelativeToParent(std::shared_ptr<const Joint> joint, int frame) const;

		/// \param frame Frame. This value must be between 0 and frames() - 1.
		Eigen::Affine3f GetTransformation(std::shared_ptr<const Joint> joint, int frame) const;

		/// \param frame Frame. This value must be between 0 and frames() - 1.
		Eigen::Affine3f GetRootTransformation(int frame) const
		{
	...
	public:
		...
		const auto& rotation() const { return rotation_; }
		auto& rotation() { return rotation_; }
		...
		const Eigen::Vector3f& end_site() const { return end_site_; }
		Eigen::Vector3f& end_site() { return end_site_; }

		const std::string& name() const { return name_; }

		const std::list<std::shared_ptr<Joint>>& children() const { return children_; }
		std::list<std::shared_ptr<Joint>>& children() { return children_; }
		...
		inline auto isSelected() const { return selected_;  };
		auto setSelected(bool b) { selected_ = b; }
	private:
		const std::string            name_;
		const std::shared_ptr<Joint> parent_;

		bool                              has_end_site_ = false;
		Eigen::Vector3f                   end_site_;
