...
namespace bvh11
{
	namespace internal
	{
		...
		inline Eigen::Vector3f read_offset(const std::vector<std::string>& tokens)
		{
			assert(tokens.size() == 4 && tokens[0] == "OFFSET");
			const float offset_x = std::stof(tokens[1]);
			const float offset_y = std::stof(tokens[2]);
			const float offset_z = std::stof(tokens[3]);
			return Eigen::Vector3f(offset_x, offset_y, offset_z);
		}
	}
	...
	std::vector<std::shared_ptr<bvh11::Joint>> BvhObject::GetJointList()
	{
	...
	Eigen::Affine3f BvhObject::GetTransformationRelativeToParent(std::shared_ptr<const Joint> joint, int frame) const
	{
		...
		if (has_time_varying_translation)
		{
			...
			for (int channel_index : joint->associated_channels_indices())
			{
				...
				const float  value = motion()(frame, channel_index);

				switch (channel.type)
				{
				case bvh11::Channel::Type::x_position:
					transform *= Eigen::Translation3f(Eigen::Vector3f(value, 0.0, 0.0));
					break;
				case bvh11::Channel::Type::y_position:
					transform *= Eigen::Translation3f(Eigen::Vector3f(0.0, value, 0.0));
					break;
				case bvh11::Channel::Type::z_position:
					transform *= Eigen::Translation3f(Eigen::Vector3f(0.0, 0.0, value));
					break;
				case bvh11::Channel::Type::x_rotation:
					transform *= Eigen::AngleAxisf(value * M_PI / 180.0, Eigen::Vector3f::UnitX());
					break;
				case bvh11::Channel::Type::y_rotation:
					transform *= Eigen::AngleAxisf(value * M_PI / 180.0, Eigen::Vector3f::UnitY());
					break;
				case bvh11::Channel::Type::z_rotation:
					transform *= Eigen::AngleAxisf(value * M_PI / 180.0, Eigen::Vector3f::UnitZ());
					break;
				}
			}
		}
		else
		{
			...
			transform *= Eigen::Translation3f(joint->offset());
	...
	Eigen::Affine3f BvhObject::GetTransformation(std::shared_ptr<const Joint> joint, int frame) const
	{
		Eigen::Affine3f transform = GetTransformationRelativeToParent(joint, frame);
